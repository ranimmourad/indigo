 ...
 * automatically falls back to the .svg placeholder of the same basename if
...
export default function ProductImage({ src, alt, className }: ProductImageProps) {
  ...
  // Build the .svg fallback path from the .jpg path.
  const fallback = src.replace(/\.(jpe?g|png|webp|avif)$/i, ".svg");
  const finalSrc = errored ? fallback : src;

  // We use <img> (not next/image) so the fallback works cleanly across both
  // the SVG placeholders and the user-uploaded JPGs without remote-config.
  return (
